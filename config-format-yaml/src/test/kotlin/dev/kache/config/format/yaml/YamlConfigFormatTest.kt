...
import dev.kache.config.annotations.Sensitive
...
class YamlConfigFormatTest {
    ...
    @Serializable
    @CommentBlock("-- Secure configuration")
    data class SecureConfig(
        @Comment("API token")
        @Sensitive
        val token: String = "super-secret-token",
    ...
    @Test
    fun `protected fields`() {
        val yaml = format.encode(SecureConfig(), SecureConfig.serializer())

        println(yaml)

        assertTrue(
            yaml.contains("token: \"super-secret-token\""),
            "Sensitive field 'token' should be revealed"
        )

        val config = format.decode(
            yaml,
            serializer = SecureConfig.serializer()
        )

        println(config.token)
        assertEquals(config.token.toString(), "***", "Sensitive field 'token' should be hidden")
    }
}
